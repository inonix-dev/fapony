...
| `mem plan-sweep` / `plan-check` ไม่เคยทำงานเลยสำหรับคนที่ใช้ `fapony init` | `commands/plan.ts` hardcode `apps/<app>/plan` **10 จุด** ทั้งที่ `store.ts` มี fallback repo เดี่ยวอยู่แล้ว → พิมพ์ "ไม่มี plan/" ทุกครั้งเงียบ ๆ · รวมศูนย์เป็น `planBase` ใน store.ts ที่เดียว แล้ว derive `plan/` + `done/` จากมัน (`done/` พี่น้อง → ไม่ต้อง re-relativize ลิงก์ในไฟล์, legacy `plan/done/` ยังรองรับ) |
