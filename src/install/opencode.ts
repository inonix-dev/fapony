...
//
// Writes fapony's generated plugins (edit hint, opt-in git-autonomy) into
...
/** Plugins fapony used to write — memory hints now owned by fael. Their bodies
 *  import exports src/hook.ts no longer has, so a leftover copy would fail to
 *  load inside OpenCode: remove ours (by owned name), never a foreign file. */
...
function removeRetiredPlugin(
  ...
  console.error(
    `  ${fileName}: ${dryRun ? "would remove" : "removed"} (memory moved to fael)`,
  );
}
...
// with the importer count, plus convention-debt lines about the file.
  ...
  return {
    "tool.execute.after": async (input, output) => {
      try {
        ...
        // Convention debt on this file — annotate only, cap 3 lines.
