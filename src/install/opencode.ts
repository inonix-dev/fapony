...
/**
 * OpenCode plugin — session-start context via `fapony hook-session-start`, the
 * OpenCode parity of Claude's SessionStart hook.
 *
 ...
 *
 * The no-mem-log guard, the cap, and the JSON shape all live in
 * cmdHookSessionStart (src/hook.ts), shared verbatim with the Claude and Codex
 * hooks, so this plugin only parses `additionalContext` back out. It used to
 * spawn `mem kickoff` itself and push stdout — but kickoff exits 0 and prints
 * its "# <path> — 0 entries" header in a repo with no mem log, which pushed
 * junk into the first dispatch of every session (bug mub2ezhi). Spawning the
 * hook instead keeps one guard, one cap, same spawn count.
 *
 * Once per session (closure Set — the factory runs once per server), silent
 * when the hook prints nothing or exits nonzero (no mem log in scope), never
 * throws. `process.execPath` is the bun running opencode itself — no PATH
 * dependency. Verified against OpenCode 1.18.29; `experimental.chat.system.
 * transform` is an experimental hook, so a rename makes this plugin quiet with
 * no error — re-check on OpenCode upgrades.
 */
export function sessionStartPluginSource(root: string): string {
  const faponyTs = JSON.stringify(join(root, "fapony.ts"));
  return `// fapony session start — injects \`fapony mem kickoff\` as context once per
// session, silent when the repo has no mem log. Annotate only: never blocks.
// Generated by \`fapony install\` — edit src/hook.ts in the fapony checkout.
  ...
  return {
    "experimental.chat.system.transform": async (input, output) => {
      try {
        ...
        seen.add(sessionID);
        const { spawnSync } = require("node:child_process");
        const p = spawnSync(process.execPath, [${faponyTs}, "hook-session-start"], {
        ...
        if (typeof ctx === "string" && ctx) output.system.push(ctx);
      } catch {
