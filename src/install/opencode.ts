...
/**
 * OpenCode plugin — session-start context via `fapony mem kickoff`, the
 ...
 * when kickoff exits nonzero or prints nothing (no mem log in scope), never
 * throws. `process.execPath` is the bun running opencode itself — no PATH
 * dependency. Logic shared via `capContext` in src/hook.ts, same one-copy
 * shape as the other three plugins.
 */
...
// Generated by \`fapony install\` — edit src/hook.ts in the fapony checkout.
import { capContext } from ${hookModule};
  ...
  return {
    "experimental.chat.system.transform": async (input, output) => {
      try {
        ...
        seen.add(sessionID);
        const { spawnSync } = require("node:child_process");
        const kick = spawnSync(process.execPath, [${faponyTs}, "mem", "kickoff"], {
          cwd: directory,
          encoding: "utf-8",
        });
        if (kick.status !== 0) return;
        const out = (kick.stdout ?? "").toString().trim();
        if (!out) return;
        output.system.push(capContext(out));
      } catch {
...
function installSessionStartPlugin(
  ...
  if (existsSync(pluginPath)) {
    ...
    if (current.includes("capContext")) {
